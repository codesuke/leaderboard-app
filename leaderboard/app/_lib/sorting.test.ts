...
describe("sortByColumn", () => {
  it("sorts ascending by a numeric column (score)", () => {
    ...
    expect(sortByColumn([high, low], "score", "asc").map((s) => s.rollNumber)).toEqual([
      "2",
      "1",
    ]);
  });
  ...
  it("sorts descending by a numeric column (score)", () => {
    ...
    expect(sortByColumn([low, high], "score", "desc").map((s) => s.rollNumber)).toEqual([
      "1",
      "2",
    ]);
  });
  ...
  it("sorts by a string column (name) case-insensitively", () => {
    ...
    expect(sortByColumn([zed, amy], "name", "asc").map((s) => s.rollNumber)).toEqual([
      "2",
      "1",
    ]);
  });
  ...
  it("sorts by rankOverall", () => {
    ...
    expect(sortByColumn([rank2, rank1], "rankOverall", "asc").map((s) => s.rollNumber)).toEqual([
      "2",
      "1",
    ]);
  });
